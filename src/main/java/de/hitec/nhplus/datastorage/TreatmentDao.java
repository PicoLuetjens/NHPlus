...
public class TreatmentDao extends DaoImp<Treatment> {
    ...
    protected PreparedStatement getCreateStatement(Treatment treatment) {
        ...
        try {
            final String SQL = "INSERT INTO treatment (pid, nid, treatment_date, begin, end, description, remark, nurseSurname, nurseFirstname, nursePhonenumber, IS_LOCKED) " +
                    "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            preparedStatement = this.connection.prepareStatement(SQL);
            preparedStatement.setLong(1, treatment.getPid());
            preparedStatement.setLong(2, treatment.getNid());
            ...
            preparedStatement.setString(8, treatment.getNurseSurname());
            preparedStatement.setString(9, treatment.getNurseFirstname());
            preparedStatement.setString(10, treatment.getNursePhonenumber());
            preparedStatement.setString(11, treatment.getIsLocked());
    ...
    protected Treatment getInstanceFromResultSet(ResultSet result) throws SQLException {
        ...
        return new Treatment(result.getLong(1), result.getLong(2), result.getLong(3),
                date, begin, end, result.getString(7), result.getString(8), result.getString(9), result.getString(10), result.getString(11), result.getString(12));
    }
    ...
    private PreparedStatement getReadAllTreatmentsOfOneNurseByNid(long nid) {
        PreparedStatement preparedStatement = null;
        try {
            final String SQL = "SELECT * FROM treatment WHERE nid = ?";
            preparedStatement = this.connection.prepareStatement(SQL);
    ...
    public List<Treatment> readTreatmentsByNid(long nid) throws SQLException {
        ResultSet result = getReadAllTreatmentsOfOneNurseByNid(nid).executeQuery();
        return getListFromResultSet(result);
    }
    ...
    protected PreparedStatement getUpdateStatement(Treatment treatment) {
        ...
        try {
            ...
            preparedStatement.setString(10, treatment.getNursePhonenumber());
            preparedStatement.setString(11, treatment.getIsLocked());
            preparedStatement.setLong(12, treatment.getTid());
